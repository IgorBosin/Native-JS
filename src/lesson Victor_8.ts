...
export function sum(...nums: Array<number>): number {
    ...
    return nums.reduce((acc: number, item) => acc + item)
}
...
//     const hypotenuse = 0.5 * Math.sqrt(4 * Math.pow(arr[1], 2) - Math.pow(arr[0], 2))
//     //...здесь пишем код.
//     // В return стоит "заглушка", чтоб typescript не ругался
//     switch (true) {
//         case a === b && b === c: {
...
//             return '00'
//         }
//         case arr[1] === arr[2] && arr[0] + arr[1] >= arr[2]: {
//             return '01'
//         }
//         default:
//             return '11'
...
export function getTriangleType(a: number, b: number, c: number): string {
    ...
    return (a === b && b === c)
        ...
        : arr[0] + arr[1] <= arr[2]
            ? '00'
            : arr[1] === arr[2] && arr[0] + arr[1] >= arr[2] && arr[1] + arr[2] >= arr[0]
                ? '01'
                : '11'
}
...
export function getSquarePositiveIntegers(array: Array<number>): Array<number> {
    ...
    for (let i = 0; i < array.length; i++) {
        if (array[i] > 0 && array[i] === Math.round(array[i])) {
